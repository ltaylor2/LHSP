...
plot_tradeoffs <- plot_tradeoff_energy + plot_tradeoff_date + plot_spacer()
...
plot_decline_hatch <- ggplot() +
                   ...
                   stat_smooth(data=filter(dat, Is_Empirical_Strategy),
...
plot_decline_energy <- ggplot() +
                   ...
                   geom_smooth(data=filter(dat, Is_Empirical_Strategy), 
                               aes(x=Foraging_Condition_Mean, y=Successful_Mean_Energy_F),
...
ggsave(filename="Plots/FIGURE_4.png", plot=plot_declines, width=7.5, height=4)
