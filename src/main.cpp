...
std::string breedingSeason(Parent& pf, Egg& egg, bool kick);
	          ...
	          std::vector<double> v_foragingMean)
{
	...
	*/
	...
	int totParamIterations = energyCombinations * v_foragingMean.size();
	...
	if (minEnergyThresh_F >= maxEnergyThresh_F) {
        continue; 
    }
    ...
    for (int k = 0; k < 2; k++) {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            std::string seasonHistory = breedingSeason(pf, egg, kick);
            //
            //
            ...
            std::string hatchResult = checkSeasonSuccess(pf, egg);	// Factorized season result
